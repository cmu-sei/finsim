...
from finsim_cc.init_script import gen_all_data, handle_current_component
import requests, random, subprocess

## ---
# @brief cc_login used by finsim-cc to login to finsim-trans
# @input url, ccUser, ccPass - login to component specified by url with cc credentials
# @return token - access token to authenticate to the component specified by url
## ---
def cc_login(url, ccUser, ccPass):
    loginForm = { "username":ccUser, "password":ccPass }
